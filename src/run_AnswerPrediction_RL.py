...
from Bert_modeling import BertForBaiduQA_Answer_Selection, BertConfig, WEIGHTS_NAME, CONFIG_NAME
from Bert_optimization import BertAdam, warmup_linear
...
def predictions(all_examples, all_features, all_results, n_best_size,
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        assert len(nbest_json) >= 1
        assert len(nbest_json) == n_best_size
        assert len(nbest_json) <=len(index_start_end)
        # try:
        #     assert len(nbest_json) == len(index_start_end)
        # except:
        #     print(len(nbest_json))
        #     print(len(index_start_end))
        # 20长度
        gamma = [pow(0.99,i+1) for i in range(n_best_size)]
        # print(b_start_logits)
        # print(b_end_logits)
        b_start_logits = torch.tensor(b_start_logits[0]).cuda()
        b_end_logits = torch.tensor(b_end_logits[0]).cuda()
        # print(b_start_logits)
        # print(b_end_logits)
        loss = 0
        for i in range(len(nbest_json)):
            ...
            start_pos = torch.tensor([index_start_end[i][0]]).cuda()
            end_pos = torch.tensor([index_start_end[i][1]]).cuda()
            # print(start_pos)
            # print(end_pos)
            current_loss = gam * rouge * cal_loss_L(b_start_logits, b_end_logits, start_pos, end_pos)
            loss += current_loss
...
def main():
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        eval_examples = read_baidu_examples(
